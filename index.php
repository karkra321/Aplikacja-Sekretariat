			...
			<center>
				<div id="greentest">	
					...
					<label> <span><b>Wielkość klasy</b></span>
						...
						<?PHP // ************************************************* ZMIANA POCZATEK **********************************************************
							if(isset($_POST['opty']) && $_POST['opty'])echo '<input type="checkbox" name="opty" checked /> Użyj prostego podzialu klas';
							else echo '<input type="checkbox" name="opty" /> Użyj prostego podzialu klas';
							// ************************************************* ZMIANA POCZATEK **********************************************************
						?>
					</label>	
...
try{
					...
					$result = mysqli_query($polaczenie,"SELECT * FROM glowna WHERE Profil = 'Technik Informatyk' || Profil = 'Technik Elektronik' || Profil = 'Technik Programista' || Profil = 'Technik Automatyk' || Profil = 'Technik Grafiki'   ORDER BY $wyb1 DESC, $wyb2, $wyb3, $wyb4");
...
//<div onclick="usuwaniejednego($row[id])" id="usunjednego"><img src="usun.bmp" height="16px" /> <span>Usuń</span></div>
	...
	if ($allUcz>=1) 
	{					
						if($ilosc_profi_info>0){ // ************************************************* ZMIANA POCZATEK **********************************************************
							if(isset($_POST['opty']) && $_POST['opty']) 
							{
								$odp[0]=simpleodp($ilosc_profi_info,$_SESSION['rozmiar']);
								$odpowiedzi[0]=simpleodpowiedzi($ilosc_profi_info,$_SESSION['rozmiar']);
							}
							else 
							{
						...
						if($ilosc_profi_elktro>0){
							if(isset($_POST['opty']) && $_POST['opty'])
							{
								$odp[1]=simpleodp($ilosc_profi_elktro,$_SESSION['rozmiar']);
								$odpowiedzi[1]=simpleodpowiedzi($ilosc_profi_elktro,$_SESSION['rozmiar']);
							}
							else
							{
						...
						if($ilosc_profi_prog>0){
							if(isset($_POST['opty']) && $_POST['opty'])
							{
								$odp[2]=simpleodp($ilosc_profi_prog,$_SESSION['rozmiar']);
								$odpowiedzi[2]=simpleodpowiedzi($ilosc_profi_prog,$_SESSION['rozmiar']);
							}
							else
							{
						...
						if($ilosc_profi_auto>0){
							if(isset($_POST['opty']) && $_POST['opty'])
							{
								$odp[3]=simpleodp($ilosc_profi_auto,$_SESSION['rozmiar']);
								$odpowiedzi[3]=simpleodpowiedzi($ilosc_profi_auto,$_SESSION['rozmiar']);
							}
							else
							{
						...
						if($ilosc_profi_graf>0){
							if(isset($_POST['opty']) && $_POST['opty'])
							{
								$odp[4]=simpleodp($ilosc_profi_graf,$_SESSION['rozmiar']);
								$odpowiedzi[4]=simpleodpowiedzi($ilosc_profi_graf,$_SESSION['rozmiar']);
							}
							else
							{
						...
						for($k=0;$k<$odpowiedzi[2][2];$k++){
							$checksum+=$odp[2][$k];
						}	
						for($l=0;$l<$odpowiedzi[3][2];$l++){
							$checksum+=$odp[3][$l];
						}	
						for($m=0;$m<$odpowiedzi[4][2];$m++){
							$checksum+=$odp[4][$m];
						}
			if($checksum==$allUcz)
			{
					for($x=0;$x<5;$x++){	
						...
						for ($z = 1; $z <= $odp[$x][$y]; $z++) 
						{
							if($odp[$x][$y]>20)if(round($odp[$x][$y]/2) == $z-1){
								$grupa="Grupa II";		
							...
							$row = mysqli_fetch_assoc($result);
								...
								if($odp[$x][$y]>20)if($grupa=="Grupa II")$nazwaSubTabeli = $litera.$pro[$x]."gr2";
