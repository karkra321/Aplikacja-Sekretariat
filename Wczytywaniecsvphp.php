...
try{
		...
		if($polaczenie->errno !=0) throw new Exception($polaczenie);
			...
			if (($handle = fopen($filebase, "r")) !== FALSE) {  // sprawdzanie czy plik sie otwiera + otwieranie pliku
				...
				$wantedtable[3]="Miejscowość";					//Tworzenie stalej tablicy z szukanymi kolumnami 
				$wantedtable[4]="Ulica";
				$wantedtable[5]="Nr budynku";
				$wantedtable[6]="Nr mieszkania";
				$wantedtable[7]="angielski";
				$wantedtable[8]="niemiecki";
				$wantedtable[9]="Nazwa zawodu";
							...
							for($col=0;$col<licz_rzedy();$col++)
								{
									...
									$numer_mieszkania=$dane[6][$col];
									$Angol=$dane[7][$col];
									$Ocena_Niemiecki=$dane[8][$col];
									$Profil=$dane[9][$col];
									$polaczenie->query("INSERT INTO glowna VALUES (NULL,'$name','$surname','$post_code_sized','$city','$street','$home_number','$numer_mieszkania','$Angol','$Ocena_Niemiecki','$Profil')");
								}
								fclose($handle);
			}
		}
